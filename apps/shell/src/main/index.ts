...
// Ensure ai-settings.json template file exists in userData directory on startup
const autoAiSettingsFile = join(app.getPath('userData'), 'ai-settings.json')
if (!existsSync(autoAiSettingsFile)) {
  ...
  try {
    mkdirSync(join(autoAiSettingsFile, '..'), { recursive: true })
    writeFileSync(autoAiSettingsFile, JSON.stringify(initialSettings, null, 2))
  } catch {
    /* ignore */
  }
}
